...
class Controller:
    ...
    def changesystembrightness():
        ...
        # Ensure brightness_values is a list and extract first value
        if isinstance(brightness_values, list) and brightness_values:
            currentBrightnessLv = float(brightness_values[0]) / 100.0  # Convert to fraction
        else:
            currentBrightnessLv = 0.5  # Default brightness if retrieval fails
        # Adjust brightness based on pinch level
        currentBrightnessLv += Controller.pinchlv / 50.0
        currentBrightnessLv = max(0.0, min(1.0, currentBrightnessLv))  # Clamp between 0 and 1
        # Apply brightness change
        sbcontrol.fade_brightness(int(100 * currentBrightnessLv), start=brightness_values[0])
        print(f"Brightness adjusted to: {int(100 * currentBrightnessLv)}%")  # Debugging output
